...
// @ts-expect-error
import IconDatabase from '@tabler/icons-react/dist/esm/icons/IconDatabase';
// @ts-expect-error
    ...
    <div className="admin-header-controls">
      <div style={{ marginBottom: '10px', display: 'flex', justifyContent: 'flex-end' }}>
        <button className="btn" style={{ background: 'var(--text-muted)', color: 'white', fontSize: '0.75rem', padding: '6px 12px' }} onClick={handleRestore}>
            <IconRefresh size={14} /> Restaurar (Default)
        </button>
      </div>

      <div className="grid" style={{ gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
        <div className="card" style={{ marginBottom: 0 }}>
          <h4 style={{ display: 'flex', alignItems: 'center', gap: '8px', marginTop: 0 }}><IconSettings size={18} /> Académico</h4>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '5px' }}>
              {academicOptions.map((o: any) => (
                <span key={o.id} className="tag" style={{ display: 'flex', alignItems: 'center', gap: '4px', background: 'var(--border-color)' }}>
                  {o.value} <IconX size={12} style={{ cursor: 'pointer' }} onClick={() => deleteAcademicOption(o.id)} />
                </span>
              ))}
            </div>
          </div>
      ...
      <div className="grid" style={{ gridTemplateColumns: '1fr 1fr', gap: '1rem', marginTop: '1rem' }}>
        <div className="card" style={{ marginBottom: 0 }}>
          <h4 style={{ display: 'flex', alignItems: 'center', gap: '8px', marginTop: 0 }}><IconPlus size={18} /> Nuevo Equipo</h4>
          <div style={{ display: 'flex', gap: '5px' }}>
            <input 
              type="text" 
              placeholder="Nombre..." 
        ...
        <div className="card" style={{ marginBottom: 0 }}>
          <h4 style={{ display: 'flex', alignItems: 'center', gap: '8px', marginTop: 0 }}><IconUsers size={18} /> Asignar Alumno</h4>
      ...
      <Modal opened={showRestoreConfirm} onClose={() => setShowRestoreConfirm(false)} title="Confirmar" size="xs">
        <Text size="sm" mb="md">¿ATENCIÓN: Esto borrará TODAS las evaluaciones, equipos y calificaciones actuales para restaurar los datos de users.csv?</Text>
        <Group justify="flex-end" gap="sm">
          <Button size="xs" variant="light" onClick={() => setShowRestoreConfirm(false)}>Cancelar</Button>
          <Button size="xs" color="red" onClick={async () => { setShowRestoreConfirm(false); const res = await restoreSystemDefault(); if (res.success) { notifications.show({ title: 'Sistema restaurado', message: 'Datos restaurados correctamente.', color: 'green' }); window.location.reload(); } else { notifications.show({ title: 'Error', message: res.error, color: 'red' }); } }}>Restaurar</Button>
        </Group>
      </Modal>
    </div>
  );
