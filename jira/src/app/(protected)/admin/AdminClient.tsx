...
// @ts-expect-error
import IconLayoutGrid from '@tabler/icons-react/dist/esm/icons/IconLayoutGrid';
    ...
    <div className="admin-container" style={{ padding: '20px' }}>
      <div style={{ marginBottom: '2rem' }}>
          <h1 style={{ display: 'flex', alignItems: 'center', gap: '10px', fontSize: '1.5rem', marginBottom: '1.5rem' }}>
              <IconLayoutGrid size={32} color="var(--primary-color)" /> Panel de Control Administrativo
          </h1>
          <DataPanel />
      </div>
      ...
      <h2 style={{ margin: '2.5rem 0 1.5rem 0', display: 'flex', alignItems: 'center', gap: '10px' }}>
        <IconUsers size={28} /> Gestión de Equipos
      </h2>
      ...
      <div className="teams-hierarchy">
        {Object.keys(groupedTeams).sort().reverse().map(year => (
          <div key={year} className="year-group" style={{ marginBottom: '1.5rem' }}>
            ...
            >
                {expandedYears.includes(year) ? <IconChevronDown size={20} /> : <IconChevronRight size={20} />}
                📅 Ciclo Lectivo {year}
            </button>
                ...
                {Object.keys(groupedTeams[year]).sort().map(div => {
                  ...
                  return (
                    <div key={divKey} className="div-group" style={{ marginBottom: '1rem' }}>
                        ...
                        >
                            {expandedDivs.includes(divKey) ? <IconChevronDown size={18} /> : <IconChevronRight size={18} />}
                            🏫 {div}
                        </button>
