...
// @ts-expect-error
import IconKey from '@tabler/icons-react/dist/esm/icons/IconKey';
// @ts-expect-error
...
export default function ReportsTable({ initialReports, teams, unassignedUsers }: any) {
  ...
  const handleResetPass = async (userId: number) => {
    ...
    setResetUserId(null);
    setNewPass('');
    notifications.show({ title: 'Contraseña', message: 'Actualizada con éxito.', color: 'green' });
  };
  ...
  const SortIcon = ({ column }: { column: string }) => {
    ...
    return sortConfig.direction === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />;
  };
    ...
    <div className="card" style={{ padding: '1rem' }}>
      <div style={{ display: 'flex', gap: '15px', marginBottom: '20px' }}>
        <div style={{ flex: 1, position: 'relative' }}>
          <IconSearch size={18} style={{ position: 'absolute', left: '10px', top: '50%', transform: 'translateY(-50%)', color: 'var(--text-muted)' }} />
          <input 
            type="text" 
                  ...
                  <tr key={r.id}>
                    ...
                    <td style={{ textAlign: 'center' }}>
                      {r.pending_count > 0 ? (
                        ...
                        <span style={{ color: '#38a169' }}><IconCheck size={18} /></span>
                      )}
                    </td>
                    <td>
                      {resetUserId === r.id ? (
                        <div style={{ display: 'flex', gap: '4px' }}>
                          ...
                          />
                          <button className="btn-icon" onClick={() => handleResetPass(r.id)} disabled={isResetting} style={{ color: '#38a169' }}><IconCheck size={14} /></button>
