...
INSERT IGNORE INTO settings (`key`, `value`) VALUES ('welcome_promo_enabled', '0');
INSERT IGNORE INTO settings (`key`, `value`) VALUES ('welcome_promo_quantity', '10');
...
CREATE TABLE IF NOT EXISTS suggestions (
    id INT AUTO_INCREMENT PRIMARY KEY,
    user_id INT NOT NULL,
    category ENUM('bug', 'idea', 'mejora', 'otro') DEFAULT 'idea',
    suggestion VARCHAR(255) NOT NULL,
    status ENUM('pending', 'read', 'implemented') DEFAULT 'pending',
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE
) ENGINE=InnoDB;
