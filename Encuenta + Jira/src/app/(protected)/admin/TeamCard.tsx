...
import { Trash2, Save, X, BarChart2, MessageSquare, Star, ChevronDown, ChevronUp, AlertTriangle, GraduationCap, Hash, Check } from 'lucide-react';
...
export default function TeamCard({ team, academicOptions, onOpenChart, onOpenComments, onOpenTeacherEval }: any) {
  ...
  const handleSaveExternalId = async (userId: number) => {
    ...
    setSavingId(userId);
    try {
        await updateExternalId(userId, parseInt(newId));
        const next = { ...localExternalIds };
        delete next[userId];
        setLocalExternalIds(next);
        alert("ID de Jira guardado con éxito");
    } catch (e) {
        alert("Error al guardar ID");
    } finally {
        setSavingId(null);
    }
  };
      ...
      {!isCollapsed && (
        <>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '8px' }}>
                  ...
                  <div style={{ display: 'flex', gap: '5px', alignItems: 'center', background: 'rgba(49, 130, 206, 0.05)', padding: '4px', borderRadius: '8px', border: '1px solid var(--border-color)' }}>
                      ...
                      {(localExternalIds[m.id] !== undefined) && (
                          ...
                          >
                            {savingId === m.id ? '...' : <Check size={16} strokeWidth={4} />}
                          </button>
                      )}
                  </div>
                  ...
                  <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                    ...
                    <button className="btn-icon" style={{ color: '#e53e3e', marginLeft: '5px' }} onClick={() => assignUserToTeam(m.id, null)} title="Quitar del equipo"><X size={18} /></button>
                  </div>
                </div>
              </div>
            ))}
          ...
          <div style={{ display: 'flex', gap: '8px', marginBottom: '8px' }}>
            <button className="btn btn-primary" style={{ flex: 1, fontSize: '0.85rem', padding: '10px' }} onClick={() => onOpenChart(team)}><BarChart2 size={16} /> Evolución</button>
            <button className="btn" style={{ flex: 1, background: 'var(--border-color)', color: 'var(--text-color)', fontSize: '0.85rem', padding: '10px' }} onClick={() => onOpenComments(team)}><MessageSquare size={16} /> Notas</button>
          </div>
          <button className="btn" style={{ width: '100%', background: 'rgba(159,122,234,0.15)', color: '#805ad5', border: '1px solid #805ad5', fontSize: '0.85rem', padding: '10px', fontWeight: 700 }} onClick={() => onOpenTeacherEval(team)}><Star size={16} /> Calificar Alumnos</button>
        </>
      )}
    </div>
  );
