...
export default function EvaluationWizard({ evaluatorId, evaluateeId, periodId, isSM }: any) {
  ...
  // Validación: ¿Están todas las preguntas del paso actual respondidas?
  const isStepValid = () => {
  ...
  return (
    <div className="wizard">
      ...
      <div className="step-content" style={{ minHeight: '350px' }}>
        ...
        {(step === 5 || (step === 4 && !isSM)) && (
            <div>
            <h3 style={{ color: 'var(--primary-color)', marginBottom: '20px' }}>✍️ Comentarios Finales</h3>
            <p style={{ marginBottom: '15px', fontSize: '0.9rem' }}>Escribe algo constructivo sobre el desempeño de tu compañero:</p>
            <textarea 
                rows={6} 
      ...
      <div className="wizard-actions" style={{ display: 'flex', justifyContent: 'space-between', marginTop: '30px', gap: '15px' }}>
        {step > 1 && (
        ...
        {(step < (isSM ? 5 : 4)) ? (
          ...
          >
            <CheckCircle2 size={18} /> Finalizar Evaluación
          </button>
        )}
      </div>
    </div>
