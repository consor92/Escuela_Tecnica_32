    ...
    SELECT 
        ...
        (
          SELECT COUNT(*) FROM (
            SELECT ep.id as p_id, u2.id as target_id
            FROM evaluation_periods ep
            JOIN users u2 ON u2.team_id = u.team_id AND u2.id != u.id
            WHERE ep.start_date <= CURDATE()
          ) as expected
          LEFT JOIN evaluations e2 ON e2.evaluator_id = u.id AND e2.evaluatee_id = expected.target_id AND e2.period_id = expected.p_id
          WHERE e2.id IS NULL AND u.team_id IS NOT NULL
        ) as pending_count
    FROM users u
